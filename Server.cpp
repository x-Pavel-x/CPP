...
class iskl_ili_soobshenie : public soobshenie
{
public:
	int* m;
...
public:
	...
	iskl_ili_soobshenie vhodyashie_soobsheniya[10];
	int letter_counter;
...
void WorkWithClient(int index)
{
	...
	else
	{
		...
		{
			recv(Connections[index], msg1, sizeof(msg1), NULL); 
			char comanda1[sizeoftext] = "POLYCHAI_MOE_SOOBSHENIE";
			char comanda2[sizeoftext] = "OTDAI_MOE_SOOBSHENIE";
			if (strcmp(comanda1, msg1) == 0)
			{
				iskl_ili_soobshenie Message2;
				...
				if (strcmp(User1.login, adresat_user) != 0 && strcmp(User2.login, adresat_user) != 0)
				{
					...
					{
						User1.vhodyashie_soobsheniya[User1.letter_counter] = Message2;
					...
					{
						User2.vhodyashie_soobsheniya[User2.letter_counter] = Message2;
			...
			if (strcmp(comanda2, msg1) == 0)
			{
				...
				{
					send(Connections[index], current_user.vhodyashie_soobsheniya[j].from, sizeof(current_user.vhodyashie_soobsheniya[j].from), NULL);
					send(Connections[index], current_user.vhodyashie_soobsheniya[j].text, sizeof(current_user.vhodyashie_soobsheniya[j].text), NULL);
					for (int i = 0; i < sizeoftext; i++)
					{
						symbol[0] = (char)current_user.vhodyashie_soobsheniya[j].m[i];
						send(Connections[index], symbol, sizeof(symbol), NULL);
					}
				}
			}
